...
const useStyles = /*#__PURE__*/makeStyles(() => createStyles({
  item: {
    cursor: "pointer"
  }
}));
...
const useStyles$1 = /*#__PURE__*/makeStyles(theme => createStyles({
  typingText: {
...
const useStyles$2 = /*#__PURE__*/makeStyles(() => ({
  input: {
    flex: "auto"
  },
  inputUpload: {
...
const AlertModale = _ref => {
  ...
  return useMemo(() => /*#__PURE__*/React__default.createElement(AlertDialog, {
    open: modaleInfo,
...
const StyledMenu = /*#__PURE__*/styled(props => /*#__PURE__*/createElement(Menu, _extends({
  elevation: 0,
  anchorOrigin: {
    vertical: 'bottom',
    horizontal: 'right'
...
function ConferenceButton(props) {
   ...
   */
  return /*#__PURE__*/createElement("div", null, /*#__PURE__*/createElement(Button$1, {
    ...
    onClick: handleClick,
    startIcon: /*#__PURE__*/createElement(VideoCallIcon, null),
    endIcon: /*#__PURE__*/createElement(KeyboardArrowDownIcon, null),
    disabled: visitData.length === 0
  }, t('CHAT.CONFERENCE.START')), /*#__PURE__*/createElement(ConfirmDialogSlide, {
  ...
  }), /*#__PURE__*/createElement(StyledMenu, {
    id: "conference-menu",
    MenuListProps: {
    ...
    return /*#__PURE__*/createElement(MenuItem, {
      ...
      value: item.visitId,
      disableRipple: true
    }, item.conferenceStatus === 'finished' ? /*#__PURE__*/createElement(RestartAltIcon, null) : /*#__PURE__*/createElement(PlayArrowIcon, null), getVisitMessage(item));
  })));
}
...
const RoomHeader = _ref => {
  ...
  if (group.groupId) {
    ...
    return /*#__PURE__*/React__default.createElement(CardHeader, {
      ...
      })), user.role === 4 && ((_group$members2 = group.members) == null ? void 0 : _group$members2.find(it => it.userId !== user.userId && it.role === 4)) && onLeaveGroup && /*#__PURE__*/React__default.createElement(IconButton, {
...
const useStyles$5 = /*#__PURE__*/makeStyles(theme => createStyles({
  fileIcon: {
    fontSize: "0.75rem"
  },
  fileBody: {
...
const useStyles$7 = /*#__PURE__*/makeStyles(theme => createStyles({
  img: {
    cursor: 'pointer',
    borderRadius: theme.spacing(1.2),
    maxWidth: 284,
...
var useStyles$8 = /*#__PURE__*/makeStyles(theme => createStyles({
  rootContact: {
    padding: 1,
...
const markPrivateMessagesRead = (state, userId) => {
  ...
  if (newState.activeRoom != null && newState.activeRoom.messages) {
    for (let i = 0; i < newState.activeRoom.messages.length; i++) newState.activeRoom.messages[i] = {
...
const useStyles$b = /*#__PURE__*/makeStyles(theme => createStyles({
  main: {
    flex: '1 1 auto',
    whiteSpace: 'nowrap',
    overflow: 'hidden',
...
const TypingBadge = /*#__PURE__*/withStyles(theme => createStyles({
  badge: {
...
const OnlineBadge = /*#__PURE__*/withStyles(theme => createStyles({
  badge: {
    backgroundColor: theme.palette.primary.main,
    boxShadow: "0 0 0 2px " + theme.palette.background.paper
  }
...
const contactAvatar = (apiUrl, contact, typing) => {
  const avatar = /*#__PURE__*/React__default.createElement(Avatar, {
    alt: contact.username,
    src: contact.avatar ? combineURLs(apiUrl, contact.avatar) : ''
  });
  const isTyping = !!(typing != null && typing.contactId) && (typing == null ? void 0 : typing.userId) === contact.userId;
...
const RoomListItem = props => {
  ...
  const avatar = chat.groupId ? /*#__PURE__*/React__default.createElement(Avatar, {
    alt: roomName,
    className: classes.avatarGroup
...
const useStyles$c = /*#__PURE__*/makeStyles(theme => ({
  root: {
    width: '100%',
    height: '100%'
  },
...
const useStyles$d = /*#__PURE__*/makeStyles(() => ({
  root: {
    width: "100%",
    height: "100%",
    borderRadius: 4
...
const useStyles$e = /*#__PURE__*/makeStyles(() => ({
  root: {
    width: "100%",
    height: "100%",
    borderRadius: 8,
...
const RestProvider = _ref => {
  ...
  const getGroupMessages = useCallback(async chat => {
    var _chat$messages2;
    const {
      groupId
    } = chat;
...
const SocketContext = /*#__PURE__*/createContext(initialContext$1);
const SocketProvider = _ref => {
  ...
  useEffect(() => {
    ...
    socket == null || socket.on('chatData', listener1);
    // listen user online
    const listener2 = res => {
      dispatch({
        type: 'USER_ONLINE',
    ...
    socket == null || socket.on('userOnline', listener2);
    // listen user offline
    const listener3 = res => {
      dispatch({
        type: 'USER_OFFLINE',
    ...
    socket == null || socket.on('userOffline', listener3);
    ...
    socket == null || socket.on('joinPrivateSocket', listener4);
    // typing
    let timer;
    const listener5 = res => {
      if (res.code) {
    ...
    socket == null || socket.on('typing', listener5);
    // revoke
    const listener6 = res => {
      if (res.code) {
        dispatch({
    ...
    socket == null || socket.on('revokeMessage', listener6);
    // set group gather
    const listener7 = res => {
      if (res.code) {
        dispatch({
    ...
    socket == null || socket.on('addGroup', listener7);
    ...
    socket == null || socket.on('addContact', listener8);
    // delete contact
    const listener10 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('deleteContact', listener10);
    // update GroupInfo
    const listener11 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('updateGroupInfo', listener11);
    // update UserInfo
    const listener12 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('updateUserInfo', listener12);
    // start Conference
    const listener13 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('startConference', listener13);
    // pause Conference
    const listener14 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('pauseConference', listener14);
    // stop Conference
    const listener15 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('stopConference', listener15);
    // add Operator
    const listener16 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('addOperator', listener16);
    // set ActiveRoom
    const listener17 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('setActiveRoom', listener17);
    // visit Data
    const listener18 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
          payload: res.msg
    ...
    socket == null || socket.on('visitData', listener18);
    return () => {
      // detatch
      socket == null || socket.off('unauthorized', listener);
      socket == null || socket.off('chatData', listener1);
      socket == null || socket.off('userOnline', listener2);
      socket == null || socket.off('userOffline', listener3);
      socket == null || socket.off('joinPrivateSocket', listener4);
      if (timer) clearTimeout(timer);
      socket == null || socket.off('typing', listener5);
      socket == null || socket.off('revokeMessage', listener6);
      socket == null || socket.off('addGroup', listener7);
      socket == null || socket.off('addContact', listener8);
      socket == null || socket.off('deleteContact', listener10);
      socket == null || socket.off('updateGroupInfo', listener11);
      socket == null || socket.off('updateUserInfo', listener12);
      socket == null || socket.off('startConference', listener13);
      socket == null || socket.off('pauseConference', listener14);
      socket == null || socket.off('stopConference', listener15);
      socket == null || socket.off('addOperator', listener16);
      socket == null || socket.off('setActiveRoom', listener17);
      socket == null || socket.off('visitData', listener18);
    };
  ...
  useEffect(() => {
    ...
    socket == null || socket.on('privateMessage', listener1);
    return () => {
  ...
  useEffect(() => {
    ...
    socket == null || socket.on('deleteGroup', listener1);
    const listener2 = res => {
      if (res.code) {
        dispatch({
          type: 'SET_ERROR',
    ...
    socket == null || socket.on('joinGroup', listener2);
    ...
    socket == null || socket.on('joinGroupSocket', listener4);
    return () => {
      socket == null || socket.off('markAsRead', listener);
      socket == null || socket.off('deleteGroup', listener1);
      socket == null || socket.off('joinGroup', listener2);
      socket == null || socket.off('joinGroupSocket', listener4);
    };
...
//   return audio;
// };
const useStyles$f = /*#__PURE__*/makeStyles(theme => ({
  root: {
    height: '100%',
    overflow: 'hidden',
    padding: 0,
...
const ChatPage = _ref => {
  ...
  const onExitActiveRoom = useCallback(() => {
    dispatch({
      type: 'SET_ACTIVE_ROOM',
      payload: {}
    });
  }, [dispatch]);
  const onNeedMoreMessages = useCallback(async chat => {
    if (chat.groupId) await getGroupMessages(chat);else await getPrivateMessages(chat);
  }, [getPrivateMessages, getGroupMessages]);
  const onMessageDelete = useCallback((chat, message) => {
  ...
  const onChangeChat = useCallback(chat => {
    dispatch({
      type: 'SET_ACTIVE_ROOM',
      payload: {
        groupId: chat == null ? void 0 : chat.groupId,
  ...
  const onEnterRoom = useCallback(chat => {
  ...
  const onConferencePause = useCallback(conference => {
    if ((conference == null ? void 0 : conference.id) != null) socket == null || socket.emit('pauseConference', {
      id: conference.id
    });
  }, [socket == null ? void 0 : socket.id]);
  const onConferenceCallAccept = useCallback(conference => {
  ...
  const onOperatorAdd = useCallback((group, operator) => {
    socket == null || socket.emit('addOperator', {
      groupId: group.groupId,
      operatorId: operator.userId
    });
  }, [socket == null ? void 0 : socket.id]);
  const onLeaveGroup = useCallback(group => {
  ...
  const GetConferenceCall = () => state.conference.data && /*#__PURE__*/createElement(ConferenceCall, {
    apiUrl: apiUrl,
  ...
  const GetConference = () => /*#__PURE__*/createElement(Conference, {
    conference: state.conference.data,
    onClose: onConferencePause,
    langCode: state.user.langCode
  });
  const Gonf = () => state.conference.joined ? /*#__PURE__*/createElement(Fragment, null, /*#__PURE__*/createElement(GetConference, null), /*#__PURE__*/createElement(Box, {
