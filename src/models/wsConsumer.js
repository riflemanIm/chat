...
class WebSocketConsumer extends WsCommon {
    constructor(props) {
        ...
        try {
            ...
            const { ws_url, api_url, access_token, target_token, debug = false } = props;
            if (typeof ws_url !== 'undefined' && typeof access_token !== 'undefined') {
                this.wsClient = new WebSocketClient({ ws_url, api_url, access_token, target_token, debug });
                this.debug = debug;
                this.subscribe();
            } else {
                throw new Error('WebSocketConsumer Error: WS URL is empty');
            }
        } catch (error) {
    ...
    subscribe() {
        ...
        this.wsClient.on('fileUploadProgress', ({ requestId, event }) => {
            this.emit('fileUploadProgress', { requestId, percent: (event.loaded / event.total) })
        });
    ...
    sendMessage({ text, files, requestId, targetToken }) {
        this.parseOutgoingAction({ action: 'SEND_MESSAGE', payload: { text, files, requestId, targetToken } });
    }

    uploadFile({ file, requestId }) {
        this.parseOutgoingAction({ action: 'UPLOAD_FILE', payload: { file, requestId }, type: 'file' });
    }
    ...
    markMessagesAsReceived(id) {
        this.parseOutgoingAction({ action: 'MARK_MESSAGES', payload: { id, status: 'received' } });
    }

    markMessagesAsRead(id) {
        this.parseOutgoingAction({ action: 'MARK_MESSAGES', payload: { id, status: 'read' } });
    }
    ...
    parseOutgoingAction(data) {
        try {
            ...
            this.debugConsole('[WsConsumer] Error: Outgoing actions parsing failed: Bad action identifier ', error.message, data);
        }
    }
    ...
    validateOutgoindData({ action, payload }) {
        if (!this.outgoingActions.includes(action)) {
            this.debugConsole('[WsConsumer] Error: Outgoing action is not found: ', action);
            return false;
        }
        const rulesForActions = {
            ...
            DELETE_MESSAGES: () => {
                if (typeof payload === 'string' || (typeof payload === 'object' && typeof payload.length === 'number')) { // payload: string | array[string]
                    return true;
                }
            },
            MARK_MESSAGES: () => {
        ...
        return typeof rulesForActions[action] === 'function' && rulesForActions[action]() === true;
    }
}
