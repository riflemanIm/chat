...
import {
    fetchChatMessagesSuccess, fetchChatMessagesError,
    sendChatMessageSuccess, sendChatMessageError,
    uploadFileToChatMessageSuccess, uploadFileToChatMessageError,
...
class ChatMessages {
    ...
    upload = (payload) => {
        ...
        return request
            .post({
                url: `chats/9416695019/upload?userId=${params.user || 1}&user=user ${params.user || 1}`,
                data: payload,
            })
            .then((response) => (
                // const normalizedData = this.normalizer(response.data);
