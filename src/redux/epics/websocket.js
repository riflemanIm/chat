...
import {
  ...
  //connectToWebSocketError,
  CONNECT_TO_WEB_SOCKET_SUCCESS,
  //CONNECT_TO_WEB_SOCKET_ERROR,
  WebSocketConnected,
  //WEBSOCKET_CONNECTED,
  //configWebsocket,
  CONFIG_WEBSOCKET,
...
const connectToWebsocketEpic = (action$) =>
  action$.pipe(
    ofType(CONNECT_TO_WEB_SOCKET),
    mapTo(
      (() => {
        const params = qs.parse(window.location.search.substring(1));
        console.log("params !!!!!!!!!!", params.target_token);
        ...
        wsClient.on("messagesReceived", (data) => {
          ...
          const message = data.messages.find(
            (m) => m.status !== "read" && m.userId !== userId
          );
          if (typeof message !== "undefined") {
            wsClient.markMessagesAsRead(message.id);
          }
          store.dispatch(
            fetchChatMessagesSuccess({ data: { messages: data.messages } })
          );
        });
...
const configWebsocketEpic = (action$) =>
  action$.pipe(ofType(CONFIG_WEBSOCKET), mapTo((() => WebSocketConnected())()));

const connectToWebsockeSuccesstEpic = (action$) =>
  action$.pipe(
