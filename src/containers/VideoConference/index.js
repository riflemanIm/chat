...
@connect(
  ...
  (dispatch) => ({
    ...
    setConferenceToken: (data) =>
      dispatch(callActions.setConferenceToken(data)),
    ...
    unsetFileDropzoneActive: () =>
      dispatch(mediaActions.unsetFileDropzoneActive()),
...
class MessengerContainer extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      user.authStatus === AUTH_STATUS_SUCCESS &&
      user.user !== null &&
      chatUsers.length === 0
    ) {
      // fetchChatUsers();
  ...
  render() {
    const {
      app: { status },
      user: { authStatus },
      call: { conferenceId, conferenceStatus, url },
      //   chatRooms,
    } = this.props;
    ...
    if (conferenceStatus === 404) {
      return <div className="warning error">Конференция не найдена</div>;
    }
      ...
      return (
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: getLocaleName("chatIsNotActivated"),
          }}
        />
      );
    }
    ...
    return (
      <VideoConference
        ...
        url={url}
        video={
          typeof conferenceId !== "undefined" && conferenceStatus === "active"
        }
      />
    );
  }
